...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    const savedText = sessionStorage.getItem("survey_v1_text_answers");
    const savedImage = sessionStorage.getItem("survey_v1_image_answers");

    if (savedText) setTextData(JSON.parse(savedText) as TextStore);
    if (savedImage) setImageData(JSON.parse(savedImage) as ImageStore);

    void logEvent("thank_you_page_view", { page: "/survey_v1/thank-you", survey_version: "survey_v1" });
  }, []);
  ...
  async function onSubmitAll() {
    ...
    try {
      ...
      const res = await fetch("/api/submit_v1", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
      ...
      await logEvent("final_submission_response", { ok: json.ok, survey_version: "survey_v1" });

      if (json.ok) {
        sessionStorage.removeItem("survey_v1_text_answers");
        sessionStorage.removeItem("survey_v1_image_answers");
        await logEvent("thank_you_submit_success", {
          redirect_to: "/survey_v1/done",
          survey_version: "survey_v1",
        });
        window.location.href = "/survey_v1/done";
      }
    } finally {
      setSubmitting(false);
    }
